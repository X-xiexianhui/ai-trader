...
本项目以验证模型效果为核心目标，按照最简单的路线实现以下核心功能，不拓展其他复杂的功能，不考虑如何部署。
使用面向对象风格，要求高内聚低耦合
...
- 数据频率：5分钟K线
- 数据维度：OHLC（开高低收）
- 数据源：雅虎金融，获取数据并非本项目的核心功能，在测试层中实现获取获取数据的功能
...
#### 2.2.1 数据层
- **数据清洗模块**：只处理缺失值、异常值、时间对齐，不实现数据下载和管理
...
#### 2.3.1 核心框架
- **深度学习框架**：PyTorch 2.9.0, CUDA 12.6, Python 3.11.14
